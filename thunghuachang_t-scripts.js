// =======================================================
// เนื้อหาทั้งหมดในไฟล์ thunghuachang_t-scripts.js (ฉบับสมบูรณ์)
// =======================================================
...
const CLOUDINARY_CONFIG = {
    ...
    folder: 'nt-engineering/thunghuachang_t', // โฟลเดอร์ใน Cloudinary
    maxFiles: 10
};
...
// 1. ดึงชื่อไฟล์จาก URL อัตโนมัติ (เช่น "thunghuachang_t.html" -> จะได้ "thunghuachang_t")
 ...
 * ให้วางฟังก์ชันนี้ไว้ในไฟล์ thunghuachang_t-scripts.js ด้วยครับ
 */
...
async function loadThunghuachangTData() {
    try {
        const res = await fetch(API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function createImageItemHTML(image) {
    ...
    // 💡 NEW: ใช้ URL ที่ถูกตรวจสอบแล้ว (img.url คือ finalUrl จาก loadThunghuachangTData)
...
function openCloudinaryWidget() {
    ...
    if (CLOUDINARY_CONFIG.cloudName === 'YOUR_CLOUD_NAME_HERE' || 
        ...
        alert('⚠️ กรุณาแก้ไข CLOUDINARY_CONFIG ใน thunghuachang_t-scripts.js ก่อน');
        return;
    }
...
window.addEventListener('DOMContentLoaded', () => {
    loadEquipmentCounts();
    loadThunghuachangTData(); // ✅ แก้ชื่อฟังก์ชันให้ตรงกับที่เราเปลี่ยนในจุดที่ 2
    initToggleViewMore();
});
