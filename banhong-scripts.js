// =======================================================
// เนื้อหาทั้งหมดในไฟล์ nikhom-scripts.js (ฉบับสมบูรณ์)
// =======================================================

// ✅ [FIX #JS-1] ยกเลิก Firebase — ใช้ PHP API แทน (เหมือน maeta_t-scripts.js)
const API_URL = 'api_station_detail.php';
console.log("✅ ใช้ MySQL/PHP API แทน Firebase");

const CLOUDINARY_CONFIG = {
    cloudName: 'dtsx2jqzl',
    uploadPreset: 'nt_engineering',
    folder: 'nt-engineering/banhong', // ✅ [FIX #JS-2] แก้ folder ให้ตรงกับชื่อชุมสาย
    maxFiles: 10
};


// ========================================================
// 🧠 ส่วนควบคุมอัตโนมัติ (Universal Brain)
// ========================================================

const stationID = window.location.pathname.split("/").pop().replace(".html", "");
console.log("📍 ระบบกำลังโหลดข้อมูลของสถานี:", stationID);

// ✅ [FIX #JS-2b] ลบ BANHONG_DOC_REF (Firestore) ออก — ใช้ stationID + API_URL แทน

let currentModalImages = [];



// ✅ [FIX #JS-3] ใช้ PHP API แทน Firestore
async function loadEquipmentCounts() {
    try {
        ...
        Object.keys(uiMapping).forEach(id => {
            ...
            if (element) element.textContent = uiMapping[id];
            else console.warn(`⚠️ ไม่พบ element: ${id}`);
        });
 ...
 * ให้วางฟังก์ชันนี้ไว้ในไฟล์ nikhom-scripts.js ด้วยครับ
 */
...
// ✅ [FIX #JS-4] ใช้ PHP API แทน Firestore (เหมือน loadMaetaTData)
async function loadBanhongData() {
    try {
        const res = await fetch(API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
// ✅ [FIX #JS-SIDEBAR-1] เริ่มต้น sidebar ปิดทั้ง mobile และ desktop (เหมือน maeta_t)
...
// ✅ [FIX #JS-SIDEBAR-2] ใช้ isCollapsed + แยก logic desktop/mobile อย่างชัดเจน (เหมือน maeta_t)
...
async function saveAddress() {
    ...
    // ✅ [FIX #JS-5] เปลี่ยนจาก BANHONG_DOC_REF.set() → fetch PHP API
    try {
        const res = await fetch(API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function deleteAddress() {
    // ✅ [FIX #JS-6] เปลี่ยนจาก BANHONG_DOC_REF.set() → fetch PHP API
...
async function saveCoordinate() {
    ...
    if (lat && lng) {
        // ✅ [FIX #JS-7] เปลี่ยนจาก BANHONG_DOC_REF.set() → fetch PHP API
        try {
            const res = await fetch(API_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function deleteCoordinate() {
    // ✅ [FIX #JS-8] เปลี่ยนจาก BANHONG_DOC_REF.set() → fetch PHP API
...
function openCloudinaryWidget() {
    ...
    if (CLOUDINARY_CONFIG.cloudName === 'YOUR_CLOUD_NAME_HERE' || 
        ...
        alert('⚠️ กรุณาแก้ไข CLOUDINARY_CONFIG ใน nikhom-scripts.js ก่อน');
        return;
    }
...
async function saveImageGallery() {
    // ✅ [FIX #JS-9] เปลี่ยนจาก BANHONG_DOC_REF.set() → fetch PHP API
    try {
        const imagesToSave = currentModalImages.map(img => {
            const cleanImg = { ...img };
            if (cleanImg.url && cleanImg.url.includes('via.placeholder.com')) delete cleanImg.url;
            return cleanImg;
        }).filter(img => img.id);
...
async function deleteImageFromMainGallery(id) {
    // ✅ [FIX #JS-10] เปลี่ยนจาก BANHONG_DOC_REF.set() → fetch PHP API
...
async function saveAllInfo() {
    const updatedData = {
        ...
        // ✅ [FIX #JS-12] ลบ firebase.firestore.FieldValue.serverTimestamp() ออก
    };
...
// ========================================
// ✅ [FIX #JS-13] Helper: safe JSON parse
// ========================================
