// =======================================================
// เนื้อหาทั้งหมดในไฟล์ khuntannoi_r-scripts.js (ฉบับสมบูรณ์)
// =======================================================
...
const CLOUDINARY_CONFIG = {
    ...
    folder: 'nt-engineering/khuntannoi_r', // โฟลเดอร์ใน Cloudinary
    maxFiles: 10
};
...
// 1. ดึงชื่อไฟล์จาก URL อัตโนมัติ (เช่น "khuntannoi_r.html" -> จะได้ "khuntannoi_r")
 ...
 * ให้วางฟังก์ชันนี้ไว้ในไฟล์ khuntannoi_r-scripts.js ด้วยครับ
 */
...
async function loadKhuntannoiRData() {
    try {
        const res = await fetch(API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
function createImageItemHTML(image) {
    ...
    // 💡 NEW: ใช้ URL ที่ถูกตรวจสอบแล้ว (img.url คือ finalUrl จาก loadKhuntannoiRData)
...
function openCloudinaryWidget() {
    ...
    if (CLOUDINARY_CONFIG.cloudName === 'YOUR_CLOUD_NAME_HERE' || 
        ...
        alert('⚠️ กรุณาแก้ไข CLOUDINARY_CONFIG ใน khuntannoi_r-scripts.js ก่อน');
        return;
    }
...
window.addEventListener('DOMContentLoaded', () => {
    loadEquipmentCounts();
    loadKhuntannoiRData(); // ✅ แก้ชื่อฟังก์ชันให้ตรงกับที่เราเปลี่ยนในจุดที่ 2
    initToggleViewMore();
});
