// =======================================================
// เนื้อหาทั้งหมดในไฟล์ lamphun1-scripts.js (ฉบับสมบูรณ์)
// =======================================================

// ✅ [FIX #JS-1] ยกเลิก Firebase — ใช้ PHP API แทน (เหมือน maeta_t-scripts.js)
const API_URL = 'api_station_detail.php'; // PHP endpoint สำหรับหน้า station detail
console.log("✅ ใช้ MySQL/PHP API แทน Firebase");

// 🔥 Cloudinary Configuration (จำเป็นสำหรับการอัปโหลดรูปภาพ)
// ⚠️ แก้ไข folder ให้ตรงกับชื่อชุมสาย
const CLOUDINARY_CONFIG = {
    cloudName: 'dtsx2jqzl',          // ⬅️ Cloud Name ของคุณ
    uploadPreset: 'nt_engineering',  // ⬅️ Upload Preset ของคุณ
    folder: 'nt-engineering/lamphun1', // โฟลเดอร์ใน Cloudinary (แก้ตรงนี้ให้ตรงกับชื่อชุมสาย)
    maxFiles: 10
};
...
// 1. ดึงชื่อไฟล์จาก URL อัตโนมัติ (เช่น "lamphun1.html" -> จะได้ "lamphun1")
...
// ✅ [FIX #JS-2] ลบ LAMPHUN1_DOC_REF (Firestore) ออก — ใช้ stationID + API_URL แทน
...
// 2. ฟังก์ชันดึงสถิติอุปกรณ์ — ✅ [FIX #JS-3] ใช้ PHP API แทน Firestore
 ...
 * ให้วางฟังก์ชันนี้ไว้ในไฟล์ lamphun1-scripts.js ด้วยครับ
 */
...
// 3. ฟังก์ชันดึงข้อมูลสถานี — ✅ [FIX #JS-4] ใช้ PHP API แทน Firestore (เหมือน loadMaetaTData)
async function loadLamphun1Data() {
    try {
        const res = await fetch(API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (result.success && result.data) {
            ...
            // 4. อัปเดตข้อมูลรายละเอียด (dbMap) — ✅ [FIX #JS-4b] รองรับ key ทั้งรูปแบบเก่าและใหม่
            const dbMap = {
                'val-id': data['รหัส10หลัก'],
                'val-sector': data['ภาคขายและบริการ'],
                'val-short': data['ชื่อย่อสถานที่'] || data['ชื่อย่อ'],
                'val-th': data['ชื่อสถานที่ไทย'] || data['ชื่อสถานที่(ไทย)'],
                'val-th-old': data['ชื่อสถานที่ไทยเดิม'] || data['ชื่อสถานที่(ไทย)เดิม'],
                'val-en': data['ชื่อสถานที่อังกฤษ'] || data['ชื่อสถานที่(อังกฤษ)'],
                'val-en-old': data['ชื่อสถานที่อังกฤษเดิม'] || data['ชื่อสถานที่(อังกฤษ)เดิม'],
                'val-comp': data['ชื่อบริษัท'],
                'val-status': data['สถานะ'],
                'val-homing': data['Homing'],
                'val-center': data['ศูนย์บริการลูกค้า'] || data['ศูนย์บริการ'],
                'val-rank': data['Rank'],
                ...
                'val-owner': data['SITE_OWNER'] || data['SITE OWNER'],
                'val-loc': data['สถานที่ติดตั้ง'],
                'val-soi': data['ซอย'],
                'val-road': data['ถนน'],
                'val-village': data['หมู่บ้าน'],
                'val-tambon': data['แขวง/ตำบล'] || data['แขวงตำบล'],
                'val-amphoe': data['เขต/อำเภอ'] || data['เขตอำเภอ'],
                'val-province': data['จังหวัด'],
                'val-zip': data['รหัสไปรณีย์'] || data['รหัสไปรษณีย์'],
                'val-lat': data['Lat'],
                'val-long': data['Long'],
            ...
            // กรณีไม่พบข้อมูล — รีเซ็ตค่าทั้งหมด
            toggleLocationUI('address', false, null);
            toggleLocationUI('coord', false, null);
            updateMainGalleryUI();
            document.querySelectorAll('[id^="val-"]').forEach(span => span.textContent = '-');
...
// ✅ แก้ไข #3: เดิมปิด sidebar เฉพาะ mobile — แก้เป็นปิดทุกขนาดจอ
...
toggleBtn.addEventListener('click', function() {   // Toggle sidebar
    ...
    // ✅ แก้ไข #4: เดิม toggle body class ตรงๆ ทุกครั้ง ทำให้ state คลาดเคลื่อน
    //             แก้เป็น ใช้ isCollapsed เป็นตัวตัดสิน และแยก desktop/mobile
    ...
    const modalHTML = `
        ...
        ">
            <div style="
                ...
                overflow-y: auto;
            ">
                <h3 style="margin: 0 0 20px 0;">ที่อยู่ (รายละเอียด)</h3>
...
async function saveAddress() {
    ...
    // ✅ [FIX #JS-5] เปลี่ยนจาก LAMPHUN1_DOC_REF.set() → fetch PHP API
    try {
        const res = await fetch(API_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
async function deleteAddress() {
    // ✅ [FIX #JS-6] เปลี่ยนจาก LAMPHUN1_DOC_REF.set() → fetch PHP API
...
async function saveCoordinate() {
    ...
    if (lat && lng) {
        // ✅ [FIX #JS-7] เปลี่ยนจาก LAMPHUN1_DOC_REF.set() → fetch PHP API
        try {
            const res = await fetch(API_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
async function deleteCoordinate() {
    // ✅ [FIX #JS-8] เปลี่ยนจาก LAMPHUN1_DOC_REF.set() → fetch PHP API
...
function createImageItemHTML(image) {
    ...
    // 💡 NEW: ใช้ URL ที่ถูกตรวจสอบแล้ว (img.url คือ finalUrl จาก loadstationsData)
...
function openCloudinaryWidget() {
    ...
    if (CLOUDINARY_CONFIG.cloudName === 'YOUR_CLOUD_NAME_HERE' || 
        ...
        alert('⚠️ กรุณาแก้ไข CLOUDINARY_CONFIG ใน lamphun1-scripts.js ก่อน');
        return;
    }
...
async function saveImageGallery() {
    // ✅ [FIX #JS-9] เปลี่ยนจาก LAMPHUN1_DOC_REF.set() → fetch PHP API
    try {
...
async function deleteImageFromMainGallery(id) {
    // ✅ [FIX #JS-10] เปลี่ยนจาก LAMPHUN1_DOC_REF.set() → fetch PHP API
...
async function saveAllInfo() {
    const updatedData = {
        ...
        // ✅ [FIX #JS-12] ลบ firebase.firestore.FieldValue.serverTimestamp() ออก (ไม่ใช้ Firebase แล้ว)
    };
...
// ========================================
// ✅ [FIX #JS-13] Helper: safe JSON parse (ป้องกัน PHP ส่ง HTML กลับมา)
// ========================================
...
// ========================================
// ✅ [FIX #JS-14] ควบคุมการแสดงผลปุ่ม Admin (แทน Firebase onAuthStateChanged)
// ========================================
...
// ✅ [FIX #JS-15] แทนที่ firebase.auth().signOut() → เรียก api_logout.php
...
// ✅ [FIX #JS-16] แทนที่ firebase.auth().onAuthStateChanged → ตรวจสอบ PHP Session
